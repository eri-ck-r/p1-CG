...
{
  GLFWwindow* Window; ImGuiContext* Context;
};
...
static void ImGui_ImplGlfw_ContextMap_Remove(GLFWwindow* window)
{
  ...
  {
    g_ContextMap.erase_unsorted(&entry); return;
  }
}
static ImGuiContext* ImGui_ImplGlfw_ContextMap_Get(GLFWwindow* window)
{
  for (ImGui_ImplGlfw_WindowToContext& entry : g_ContextMap) if (entry.Window == window) return entry.Context; return nullptr;
}
